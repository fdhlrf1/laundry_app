    ...
    <div class="flex-1 overflow-y-auto">
        <div class="py-2">
            <div class="px-6 mx-auto max-w-7xl sm:px-6 md:px-6">
                ...
                <form action="{{ route('kelolapengguna') }}" method="GET">
                        ...
                        <div class="flex items-center w-full max-w-2xl space-x-2">
                            ...
                            <div class="relative flex-grow">
                                ...
                                <input id="search" name="search"
                                    ...
                                    placeholder="Cari pengguna" type="search">
                            </div>
                            <!-- Search Button -->
                            <button type="submit"
                ...
                <button type="button"
                    ...
                    data-twe-toggle="modal" data-twe-target="#pengguna-create" data-twe-ripple-init
            ...
            <div class="bg-white rounded-lg shadow">
                <div class="p-6 border-b border-gray-200">
                    @if (Auth()->user()->role == 'super_admin')
                        <h2 class="text-lg font-medium text-gray-900">Data Pengguna</h2>
                    @elseif (Auth()->user()->role == 'admin')
                        <h2 class="text-lg font-medium text-gray-900">Data Pengguna Anda</h2>
                    @endif
                </div>
                    ...
                    <table class="min-w-full table-auto">
                        <thead>
                            <tr class="text-left bg-gray-50">
                                ...
                                <th
                                    class="px-6 py-3 text-xs font-medium tracking-wider text-gray-500 uppercase whitespace-nowrap min-w-[195px]">
                                    Nama
                                </th>
                                <th
                                    class="px-6 py-3 text-xs font-medium tracking-wider text-gray-500 uppercase whitespace-nowrap min-w-[195px]">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($FADHIL_penggunas as $FADHIL_pengguna)
                            @include('pengguna.crudpengguna.modal-create')
                                @include('pengguna.crudpengguna.modal-edit')
                                @include('pengguna.crudpengguna.modal-delete')
                                <tr>
                                    <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">
                                        {{ $loop->iteration + $FADHIL_penggunas->firstItem() - 1 }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">
                                        {{ $FADHIL_pengguna->nama }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">
                                        {{ $FADHIL_pengguna->username }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900 capitalize whitespace-nowrap">
                                        {{ $FADHIL_pengguna->role }}</td>
                                    @if (Auth()->user()->role == 'super_admin')
                                        <td class="px-6 py-4 text-sm text-gray-900 capitalize whitespace-nowrap">
                                            {{ $FADHIL_pengguna->outlet->nama ?? 'N/A' }}</td>
                                    @endif
                                    <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">
                                        <button type="button"
                                            ...
                                            data-twe-target="#pengguna-edit{{ $FADHIL_pengguna->id }}"
                                        ...
                                        <button type="button"
                                            ...
                                            data-twe-toggle="modal"
                                            data-twe-target="#pengguna-hapus{{ $FADHIL_pengguna->id }}"
                                            data-twe-ripple-init data-twe-ripple-color="light">
                                            Hapus
                                        </button>
                                    </td>
